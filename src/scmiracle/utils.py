...
def save_tensor_to_csv(data: torch.Tensor, filename: str, delimiter: str = ',' ,index: bool = False, header: bool = False):
    """
    ...
    if index or header:
        ...
        df.to_csv(filename, sep=delimiter, index=index, header=header)
        return
...
def calculate_loss_scale(replay_cell_counts, current_cell_counts):
    """
    Calculates scaling factors to balance the loss contribution between
    historical (replay) and current data during continual learning.

    This ensures that the smaller replay buffer is not overshadowed by the
    larger new data batch during training.
    ...
    Returns:
        list: A list of weights for all batches.
    """
    total_replay = sum(replay_cell_counts)
    total_current = sum(current_cell_counts)

    # Normalize replay and current counts to sum to 1 respectively
    replay_scale = [count / total_replay for count in replay_cell_counts] if total_replay > 0 else []
    current_scale = [count / total_current for count in current_cell_counts] if total_current > 0 else []

    return replay_scale + current_scale

def plot_umap(pred, labels, dim=32, shuffle=True):
    """
    Generates and displays a UMAP visualization from model predictions.

    Args:
        pred (dict): The prediction output dictionary from the MIDAS model.
        labels (np.array): An array of cell type labels corresponding to the predictions.
        dim (int): Dimension of variables to plot UMAP.
        shuffle (bool): To shu
    """
    print("Visualizing results with UMAP...")
    adata = sc.AnnData(pred['z']['joint'][:, :dim])
    adata.obs['batch'] = pred['s']['joint'].flatten().astype(int).astype(str)
