...
class MIDAS(L.LightningModule):
    ...
    def update(model1: nn.Module, model2: nn.Module):
        """
        ...
        for name, param2 in model2_state_dict.items():
            if name in model1_state_dict:
                ...
                if isinstance(param1, torch.Tensor) and isinstance(param2, torch.Tensor):
                    ...
                    slice_tuple = tuple(slice(0, dim) for dim in min_dims)

                    param1[slice_tuple].copy_(param2[slice_tuple])
                    logging.debug(f"Updated parameter '{name}' with left-aligned weights.")
                else:
                    if param1.shape == param2.shape:
                        param1.copy_(param2)
                    else:
                        logging.debug(f"Warning: Buffer '{name}' has different shapes ({param1.shape} vs {param2.shape}), skipping direct copy.")
            else:
                logging.debug(f"Warning: Parameter '{name}' from model2 not found in model1.")

        model1.load_state_dict(model1_state_dict)
        logging.info("Model1 updated successfully with weights from model2 (left-aligned strategy).")
        return model1
    ...
    def configure_data(
        ...
        if hasattr(cls, 'net'):
            logging.info('Loading pre-defined network structure...')
            net = VAE(cls.dims_x, cls.dims_s, **cls.configs)
            cls.net = cls.update(net, cls.net)
        else:
            logging.info('Defining new network structure...')
            cls.net = VAE(dims_x, cls.dims_s, **cls.configs)
        cls.dsc = Discriminator(dims_x, cls.dims_s, **cls.configs)
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        if self.sampler_type == 'cl':
            sampler = MultiBatchContinualLearningSampler(
                dataset, 
                batch_size=self.batch_size, 
                # n_max=self.n_max, 
                n_current_datasets=self.current_batches, 
                n_replay_datasets=self.replay_batches
                )
        else:
    ...
    def training_step(self, 
        ...
        if self.train_mod == 'offline':
            scale = 1.0
        else:
            scale = self.scale[batch['s']['joint'][0][0]]
        logging.debug(f'Scale: {scale:.4f}')

        # Train discriminator for n_iter_disc iterations
        for _ in range(self.n_iter_disc):
            self.train_discriminator(c_all, batch['s'], scale)
        ...
        self.update_model(loss_net*scale, self.net, self.net_optim, self.grad_clip)
    ...
    def train_discriminator(self, 
        ...
        loss_dsc = self.calc_dsc_loss(s_pred, targets) * self.lam_dsc * scale
        self.update_model(loss_dsc, self.dsc, self.dsc_optim, self.grad_clip)

    def pack_data(
            self,
            des_dir,
            subsample_num: list = None,
            subsample='random',
            format='mtx',
        ...
        if os.path.exists(des_dir):
            logging.warning(f'Directory {des_dir} already exists. Contents may be overwritten.')

        # Determine if proportional sampling should be performed.
        all_indices = []
        if subsample_num is not None and subsample == 'BTS':
            if sum(len(self.datalist[i]) for i in range(len(self.datalist))) > sum(subsample_num):
                z = self.predict()['z']['joint'][:, :self.dim_c]
                for batch in range(len(self.datalist)):
                    if subsample_num[batch] < len(self.datalist[batch]):
                        start_idx = sum(len(self.datalist[i]) for i in range(batch))
                        end_idx = start_idx + len(self.datalist[batch])
                        id = sorted(BallTreeSubsample(z[start_idx:end_idx], subsample_num[batch]))
                        all_indices.append([int(x) for x in id])
            else:
                for batch in range(len(self.datalist)):
                    all_indices.append(list(range(len(self.datalist[batch]))))
        elif subsample_num is not None and subsample in ['random']:
            for batch in range(len(self.datalist)):
                if subsample_num[batch] < len(self.datalist[batch]):
                    all_indices.append(sorted(np.random.permutation(len(self.datalist[batch])[:subsample_num[batch]])))
                else:
                    for batch in range(len(self.datalist)):
                        all_indices.append(list(range(len(self.datalist[batch]))))
            ...
            if subsample_num is not None:
                if len(data) > 0:
                    if subsample is not None:
                        # Generate random indices and create a Subset.
                        current_data = data.__subset__(all_indices[batch_id])
                    logging.info(f"Sampling {len(current_data)} of {len(data)} items from batch {batch_id}.")
                else:
                    logging.warning(f"Batch {batch_id} is empty. Skipping.")

            if not use_transform:
                current_data.transform = {}
                
            if format in ['mtx', 'csv']:
                data_loader = DataLoader(current_data, shuffle=False, batch_size=self.batch_size, num_workers=self.num_workers)
            elif format == 'vec':
                data_loader = DataLoader(current_data, shuffle=False, batch_size=1, num_workers=self.num_workers)
                if len(current_data) > 0:
                    fname_fmt = get_name_fmt(len(data_loader)) + '.csv'
            else:
                raise ValueError(f'Unsupported format: {format}. Supported formats are "mtx", "csv", and "vec".')

            cum_data = {}
            last_batch_data = None
            for i, batch_data in enumerate(tqdm(data_loader)):
                os.makedirs(os.path.join(des_dir, f'batch_{batch_id}'), exist_ok=True)
                if format in ['mtx', 'csv']:
                    for m in batch_data['x'].keys():
                        cum_data.setdefault(m, []).append(batch_data['x'][m])
                else: # format == 'vec'
                    for m in batch_data['x'].keys():
                        os.makedirs(os.path.join(des_dir, f'batch_{batch_id}', 'vec', m), exist_ok=True)
                        save_tensor_to_csv(batch_data['x'][m], os.path.join(des_dir, f'batch_{batch_id}', 'vec', m, fname_fmt % i))
                last_batch_data = batch_data

            if format in ['mtx', 'csv']:
                if cum_data:
                    cum_data = {m: torch.cat(cum_data[m], dim=0) for m in cum_data.keys()}
                    logging.debug(f'Concatenated data {cum_data}')
                    os.makedirs(os.path.join(des_dir, f'batch_{batch_id}', 'mat'), exist_ok=True)
                    logging.info(f'Saving concatenated data for batch {batch_id}...')
                    for m in cum_data.keys():
                        if format == 'mtx':
                            save_tensor_to_mtx(cum_data[m], os.path.join(des_dir, f'batch_{batch_id}', 'mat', m + '.mtx'))
                        else:
                            save_tensor_to_csv(cum_data[m], os.path.join(des_dir, f'batch_{batch_id}', 'mat', m + '.csv'), header=True, index=True)

            # Save mask from the last processed batch, if any data was processed.
            if last_batch_data:
                os.makedirs(os.path.join(des_dir, f'batch_{batch_id}', 'mask'), exist_ok=True)
                for m in last_batch_data['e'].keys():
                    pd.DataFrame(last_batch_data['e'][m][0].numpy()).T.to_csv(os.path.join(des_dir, f'batch_{batch_id}', 'mask', m + '.csv'), header=True, index=True)
            else:
                logging.warning(f'No data was processed for batch {batch_id} (possibly due to sampling). Mask will not be saved.')

        # Save feature dimensions
        os.makedirs(os.path.join(des_dir, 'feat'), exist_ok=True)
        with open(os.path.join(des_dir, 'feat', 'feat_dims.toml'), 'w') as f:
            toml.dump(self.dims_x, f)
        
        if subsample_num is not None:
            # write json of indices
            for batch_id in range(len(all_indices)):
                with open(os.path.join(des_dir, f'batch_{batch_id}','subsample.json'), 'w') as f:
                    json.dump(all_indices[batch_id], f, indent=4)

        if subsample_num is not None and return_idx:
            return all_indices
    ...
    def predict(self, 
        ...
        with torch.no_grad():
            for batch_id, data in enumerate(tqdm(self.datalist)):
    ...
    @classmethod
    def configure_new_data_from_dir(cls,
                                configs: Dict[str, Any], 
                                dir_path: str,
                                format: str = 'mtx',
        ...
        # combine dims_x keys or keep the same
        cls.dims_x = dims_x if not hasattr(cls, 'dims_x') else {**cls.dims_x, **dims_x}
        logging.debug(f'Configured dims_x: {cls.dims_x}')
        # Configure datasets and associated parameters
        datalist, dims_s, s_joint, combs = cls.get_datasets_from_dir(data, mask, transform, format, cls.dims_x)
        cls.replay_batches = len(cls.datalist) - len(data)
        cls.current_batches = len(data)
        cls.scale = scale
       # merge transform
        transform = {**kwargs.get('transform',{}), **(transform if transform else {})}
        logging.debug(f'dims_s:{dims_s}, transform:{transform}, s_joint:{s_joint}, combs:{combs}')
        cls.start_epoch = 0
    ...
    def get_datasets_from_dir(
        ...
        """
        cls.s_joint = cls.s_joint if hasattr(cls, 's_joint') else []  # Modality indices for each batch
        cls.n_s = cls.n_s if hasattr(cls, 'n_s') else {'joint':-1}  # Counter for each modality
        cls.combs = cls.combs if hasattr(cls, 'combs') else []  # Modality combinations for each batch
        if hasattr(cls, 'datasets'):
            cls.train_mod = 'continual'
        else:
            cls.train_mod = 'offline'
        cls.datasets = cls.datasets if hasattr(cls, 'datasets') else []  # List of datasets
        cls.dims_s = cls.dims_s if hasattr(cls, 'dims_s') else {}  # Dimensions for batch correction
        for i, batch_data in enumerate(data):
            ...
            for modality in batch_data.keys():
                if modality in cls.n_s:
                    batch_s[modality] = cls.n_s[modality] + 1
                    cls.n_s[modality] += 1
                else:
                    batch_s[modality] = 0
                    cls.n_s[modality] = 0
            ...
            cls.n_s['joint'] += 1
            batch_s['joint'] = cls.n_s['joint']
            cls.s_joint.append(batch_s)
            cls.combs.append(batch_combs)
        ...
        cls.dims_s = {modality: count + 1 for modality, count in cls.n_s.items()}
        # concat mask
        cls.mask = cls.mask + mask if hasattr(cls, 'mask') else mask
        
        if cls.train_mod == 'continual':
            logging.debug(f'expected_dims{cls.dims_x}')
            for i, dataset in enumerate(cls.datasets[:-len(data)]):
                dataset_update = MultiModalDataset(
                    dataset.mod_dict, 
                    dataset.mod_id_dict, 
                    dataset.file_type, 
                    dataset.mask_path, 
                    dataset.transform, 
                    expected_dims = cls.dims_x)
                cls.datasets[i] = dataset_update
                logging.debug(f'Update dataset for batch {i} with modalities')
        MIDAS.print_info(cls.mask, cls.datasets)
    ...
    def reset(cls):
        ...
        """
        attributes_to_reset = [
            'dims_x', 'dims_s', 's_joint', 'n_s', 'combs', 
            'datasets', 'mask', 'train_mod', 'replay_batches', 'current_batches', 'datalist', 'scale'
        ]
